...
async def run_simple(session: Session, session_id: str):
    ...
    async def run_one(agent):
        ...
        provider = build_provider(agent.provider_key, agent.model_name, __import__('app.core.security', fromlist=['decrypt_value']).decrypt_value(provider_row.api_key_encrypted or '') if provider_row else '', provider_row.base_url if provider_row else None, agent.max_tokens)
        ...
        add_session_message(session, SessionMessageRecord(session_id=session_id, round_no=1, phase='INDEPENDENT', agent_name=agent.agent_name, message_type='OPINION', content=res.text, vote=parsed['vote'], confidence=parsed['confidence'], latency_ms=res.latency_ms))
        return agent.agent_name, parsed

    results = dict(await asyncio.gather(*[run_one(agent) for agent in agent_configs]))
...
async def run_debate(session: Session, session_id: str):
    ...
    provider_rows = {p.provider_key: p for p in list_provider_configs(session)}
    latest_outputs = {m.agent_name: m.content for m in session.query(SessionMessageRecord).filter(SessionMessageRecord.session_id == session_id, SessionMessageRecord.phase == 'INDEPENDENT').all()}
    ...
    async def rerun(agent_name: str, opposing_summary: str):
        ...
        add_session_message(session, SessionMessageRecord(session_id=session_id, round_no=2, phase='DEBATE', agent_name=agent.agent_name, message_type='REBUTTAL', content=res.text, vote=parsed['vote'], confidence=parsed['confidence'], latency_ms=res.latency_ms))
        return agent_name, parsed['vote']

    updated = dict(await asyncio.gather(*([rerun(name, majority_summary) for name in minority] + [rerun(name, minority_summary) for name in majority])))
