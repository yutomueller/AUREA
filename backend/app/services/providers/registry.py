...
def build_provider(provider_key: str, model_name: str, api_key: str | None, base_url: str | None, max_tokens: int, timeout_seconds: int = 60):
    cls = PROVIDER_MAP[provider_key]
    return cls(model_name=model_name, api_key=api_key, base_url=base_url, max_tokens=max_tokens, timeout_seconds=timeout_seconds)


def build_provider_from_record(record: ProviderConfigRecord, model_name: str, max_tokens: int, timeout_seconds: int = 60):
    return build_provider(record.provider_key, model_name, decrypt_value(record.api_key_encrypted or ''), record.base_url, max_tokens, timeout_seconds)
